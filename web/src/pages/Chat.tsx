...
export default function Chat() {
  ...
  const { name } = widgetConfig.institute;
        ...
        <div className="flex h-full min-w-0 flex-1 flex-col">
          <div ref={scrollRef} className="flex-1 overflow-y-auto">
            {!active || active.messages.length === 0 ? (
              <QuickStartChips
                name={name !== "Javaab" ? name : undefined}
                classNum={defaults.class_level}
                board={defaults.board.toLowerCase() as "cbse" | "gseb"}
                onPick={(t) => send(t)}
              />
            ) : (
                    ...
                    onEdit={
                      m.role === "user" && activeId
                        ? (msgId, newContent, originalContent) =>
                            sendEdit(activeId, msgId, newContent, originalContent)
                        : undefined
                    }
                    onBookmark={
          ...
          <div className="border-t border-border bg-background/80 px-3 py-3 backdrop-blur sm:px-6 sm:py-4">
            ...
            <p className="mx-auto mt-2 max-w-3xl text-center text-[11px] text-muted-foreground">
              {name} AI can make mistakes. Verify important answers with your textbook.
              {widgetConfig.branding.show_javaab_credit && (
                <> · Powered by <span className="font-semibold">Javaab</span></>
              )}
            </p>
          </div>
        </div>
